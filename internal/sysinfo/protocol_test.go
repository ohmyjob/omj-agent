...
func TestWorkMetadata(t *testing.T) {
	...
	got := info.WorkMetadata(true)

	want := protocol.MachineMetadata{Hostname: "nas01", OS: "linux", OSVersion: "Debian GNU/Linux 12", Arch: "arm64", KernelVersion: "6.1.0", AgentUser: "ohmyjob", AgentUID: 998, InsecureHTTP: true, ReportedIPs: []string{"192.168.1.20"}}
...
func TestWorkMetadataEncodesMissingAddressesAsAnEmptyList(t *testing.T) {
	encoded, err := json.Marshal(Info{Hostname: "nas01"}.WorkMetadata(false))
	if err != nil {
...
func TestEnrollRequest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := info.EnrollRequest("omj_enroll_token", tt.machine, false)
