...
- The first end-to-end run showed `cancellation requested` logged once per poll (five times for one Run) while the process was dying. The Server correctly keeps a Run in `cancel_run_ids` until its finish is accepted, so `Agent.cancel` now acts through a `sync.Once` on the `Run`: the first poll cancels and logs, later polls are silent. `Process.Cancel` was already idempotent, so only the noise changed. The stop-signal path still cancels every Run directly, because `agent_stopped` must reach a Run that no one asked to cancel.
