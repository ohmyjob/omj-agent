...
Everything an operator needs to install, run, secure and troubleshoot the Agent, written for the repo README and ohmyjob.sh.
...
- `docs/security.md`: the trust model from §21, what the service user can and cannot do, the clean per-Run environment, the `sudo` pattern for privileged Jobs with a worked example, the opt-in hardening block and what each directive breaks, running as another user, running as root and why the UI warns, disclosure to `security@ohmyjob.sh`.
