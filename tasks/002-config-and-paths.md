...
- One atomic writer (`writeFileAtomically`: sibling temp file, fsync, rename) serves both the configuration and the credential; the rename is injectable so the test proves the old file survives a failed rename and the temp file is removed.
